...
sealed interface OnBoardingIntent {
    ...
    data class VerifyPinCodeResult(val status: PinCodeStatus, val data: VerifyCode? = null, ): OnBoardingIntent
    data class NameChanged(val name: String): OnBoardingIntent
    data class SelectListItem(val position: Int): OnBoardingIntent
    data class Initialize(val entryPoint: OnboardingEntryPoint) : OnBoardingIntent
}
